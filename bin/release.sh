...
API_JSON=$(printf '{"tag_name": "%s","target_commitish": "%s","name": "%s","body": "%s","draft": %s,"prerelease": %s}' "$VERSION" "$BRANCH" "$VERSION" "$MESSAGE" "false" "false" )
...
if [ "$ID" == "null" ]; then
	echo "LOG: $(date) -- ERROR Release id: $ID"
	exit 1
else
